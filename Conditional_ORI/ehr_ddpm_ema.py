...
Original file is located at
    https://colab.research.google.com/drive/1b4uarG1gBGC-upSj89coKtvaOPp-8KFb
"""
...
'''fig_2d = px.scatter(
    proj_2d, x=0, y=1,
    color= y_train.astype(str), labels={'color': 'Treatment'}, width=800, height=800,
...
"""### Train logisticregression on imbalanced training set"""
...
disp = sm.ConfusionMatrixDisplay.from_predictions(labels, pred,
                                 cmap=plt.cm.Blues,
...
"""Hack to avoid getting disconnected:

Right click > inspect element > console > insert this code in
...
plt.plot(fpr, tpr, label='ROC (area = %0.2f)' % (roc_auc))
plt.plot([0, 1], [0, 1],'r--')
plt.title('Augemented Dataset ROC Curve')
plt.xlabel('False positive rate')
plt.ylabel('True positive rate')
plt.legend(loc="lower right",fontsize = 20)
plt.show()
plt.savefig("images/ROC_aug.png")
